...
for(i in c(1:length(inds))){
  ...
  raster::writeRaster(kd.r.1, paste0("Data/raster_ind/kde_", inds[i], ".tif"), overwrite=TRUE)
  ...
  if(class(kd.sp.i)[1]=="sf"){
    ...
    file.remove(paste0("Data/raster_ind/kde_", inds[i], ".tif"))
  }
...
write_sf(kd.out, "Data/shp/kde_individual.shp")
...
for(i in c(1:nrow(regs))){
  ...
  kd.r <- raster(kd[[1]])
  kd.r.1 <- (kd.r-minValue(kd.r))/(maxValue(kd.r)-minValue(kd.r))
  raster::writeRaster(kd.r.1, paste0("Data/raster_reg/kde_", regs$ID[i], ".tif"), overwrite=TRUE)
  ...
  for(j in 1:length(isos)){
    ...
    if(class(kd.sp.i)[1]=="sf"){
      ...
      file.remove(paste0("Data/raster/kde_", regs[i], ".tif"))
    }
...
write_sf(kd.out, "Data/shp/kde_region.shp")
